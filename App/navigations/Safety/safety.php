<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Safety - SocialLoop</title>
    <link rel="stylesheet" href="/public/css/safety.css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Volkhov:ital,wght@0,400;0,700;1,400;1,700&display=swap" rel="stylesheet">
</head>
<body>
    <!-- Header -->
    <header>
        <div class="header-container">
            <div class="header-left">
                <div class="logo"></div>
                <nav>
                    <ul>
                        <li><a href="../homepage/index.html">Home</a></li>
                        <li><a href="/App/navigations/aboutUs.html">About Us</a></li>
                        <li><a href="/App/navigations/supportUs.html">Support Us</a></li>
                        <li><a href="/App/navigations/Safety/safety.html">Safety</a></li>
                        <li><a href="/App/navigations/howitworks.html">How It Works</a></li>
                    </ul>
                </nav>
            </div>
            <div class="header-right">
                <select>
                    <option>English</option>
                    <option>Turkish</option>
                    <option>Spanish</option>
                    <option>Arabic</option>
                    <option>French</option>
                    <option>Italian</option>
                    <option>German</option>
                </select>
                <a href="../../loginScreen/loginIndex.html">
                <button class="login-btn">Log in</button>
              </a>
              <a href="../../registerScreen/registerIndex.html">
                <button class="signup-btn">Sign Up</button>
            </a>
            </div>
        </div>
    </header>

    <!-- Hero Section -->
    <section class="safety-hero">
        <div class="safety-hero-content">
            <h1>Your Safety is Our Priority</h1>
            <p class="subtitle">Guidelines and tools to ensure a secure experience</p>
        </div>
    </section>

    <!-- Safety Guidelines Section -->
    <section class="guidelines">
        <div class="container">
            <div class="guidelines-content">
                <h2>Safety Guidelines</h2>
                <p>We are committed to fostering a safe and respectful community. Please follow these guidelines:</p>
                <ul class="guidelines-list">
                    <li>✔️ Respect others' boundaries and privacy.</li>
                    <li>✔️ Report suspicious or inappropriate behavior immediately.</li>
                    <li>✔️ Avoid sharing sensitive personal information.</li>
                    <li>✔️ Meet in public places for initial hangouts.</li>
                    <li>✔️ Trust your instincts and prioritize your comfort.</li>
                </ul>
            </div>
        </div>
    </section>

    <!-- Reporting Section -->
    <section class="reporting">
        <div class="container">
            <h2>Report an Issue</h2>
            <p>If you encounter a problem, let us know so we can take immediate action.</p>
            <a href="report.html" class="report-btn">Report Now</a>
        </div>
    </section>

    <!-- Footer -->
    <footer class="footer">
        <div class="footer-container">
            <div class="footer-section logo-section">
                <h2>SocialLoop</h2>
                <p>Connects people for spontaneous hangouts and real-time socializing.</p>
            </div>
            <div class="footer-section">
                <h3>Company</h3>
                <ul>
                    <li><a href="#">About Us</a></li>
                    <li><a href="#">Careers</a></li>
                    <li><a href="#">Support Us</a></li>
                </ul>
            </div>
            <div class="footer-section">
                <h3>Contact</h3>
                <ul>
                    <li><a href="#">Help/FAQ</a></li>
                    <li><a href="#">Emergency</a></li>
                    <li><a href="#">Safety</a></li>
                </ul>
            </div>
            <div class="footer-section">
                <h3>More</h3>
                <ul>
                    <li><a href="#">Report</a></li>
                    <li><a href="#">Tips for Socializing</a></li>
                    <li><a href="#">Advices for Improve</a></li>
                </ul>
            </div>
        </div>
        <div class="footer-bottom">
            <p>All rights reserved @socialloop.co</p>
        </div>
    </footer>
</body>

    <!--pop out login form-->

    <!-- Login Popup -->
<div class="popup-overlay" id="loginPopup">
    <div class="wrapper">
        <button class="close-btn" aria-label="Close login popup"></button>
        <form action="">
            <h1>Welcome Back</h1>
            <div class="input-box">
                <input type="email" placeholder="Enter your email" required>
            </div>
            <div class="input-box">
                <input type="password" placeholder="Enter your password" required>
            </div>
            <div class="rememberMe">
                <label>
                    <input type="checkbox">
                    <span>Remember me</span>
                </label>
                <a href="#">Forgot password?</a>
            </div>
            <button type="submit" id="loginBtn">Login</button>
            <div class="register-link">
                <p>Don't have an account? <a href="#">Register now</a></p>
            </div>
        </form>
    </div>
</div>

<!-- Register Popup -->
<div class="popup-overlay" id="registerPopup">
    <div class="wrapper">
        <button class="close-btn" aria-label="Close registration popup"></button>
        <form action="">
            <h1>Create Account</h1>
            <div class="input-box">
                <input type="text" placeholder="First Name" required>
            </div>
            <div class="input-box">
                <input type="text" placeholder="Last Name" required>
            </div>
            <div class="input-box">
                <input type="email" placeholder="Email" required>
            </div>
            <div class="input-box">
                <input type="password" placeholder="Password" required>
            </div>
            <div class="input-box">
                <input type="password" placeholder="Confirm Password" required>
            </div>
            <div class="input-box">
                <input type="date" required>
                <label class="date-label"></label>
            </div>
            <div class="input-box">
                <select required>
                    <option value="" disabled selected>Select Gender</option>
                    <option value="male">Male</option>
                    <option value="female">Female</option>
                    <option value="other">Other</option>
                    <option value="prefer-not">Prefer not to say</option>
                </select>
            </div>
            <button type="submit" id="registerBtn">Create Account</button>
            <div class="login-link">
                <p>Already have an account? <a href="#">Login</a></p>
            </div>
        </form>
    </div>
</div>
<div class="secretbutton">
    <a href="/App/navigations/Safety/report.html"><button>reportpage</button></a>
</div>

<script>
    let lastScroll = 0;
 let isScrollingDown = false;
 
 window.addEventListener('scroll', () => {
     const currentScroll = window.pageYOffset;
     const header = document.querySelector('header');
     const scrollThreshold = 100; // Adjust this value as needed
     
     if (currentScroll <= 0) {
         header.classList.remove('hide');
         return;
     }
 
     if (currentScroll > scrollThreshold) { // Only start hiding after threshold
         if (currentScroll > lastScroll && !isScrollingDown) {
             // Scrolling down
             header.classList.add('hide');
             isScrollingDown = true;
         } else if (currentScroll < lastScroll && isScrollingDown) {
             // Scrolling up
             header.classList.remove('hide');
             isScrollingDown = false;
         }
     }
     lastScroll = currentScroll;
 });


 // report button

 // Add this script to your safety.html file
document.addEventListener('DOMContentLoaded', () => {
    const reportButton = document.querySelector('.report-btn');
    const loginPopup = document.getElementById('loginPopup');
    const registerPopup = document.getElementById('registerPopup');
    const closeButtons = document.querySelectorAll('.close-btn');
    const registerLinks = document.querySelectorAll('.register-link a');
    const loginLinks = document.querySelectorAll('.login-link a');

    // Show popup function
    const showPopup = (popup) => {
        popup.classList.add('active');
    };

    // Hide popup function
    const hidePopup = (popup) => {
        popup.classList.remove('active');
    };

    // Add click event to Report button
    reportButton.addEventListener('click', (e) => {
        e.preventDefault();
        showPopup(loginPopup);
    });

    // Handle close buttons
    closeButtons.forEach(button => {
        button.addEventListener('click', () => {
            hidePopup(loginPopup);
            hidePopup(registerPopup);
        });
    });

    // Switch to register popup
    registerLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            hidePopup(loginPopup);
            showPopup(registerPopup);
        });
    });

    // Switch to login popup
    loginLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            hidePopup(registerPopup);
            showPopup(loginPopup);
        });
    });

    // Close popup when clicking outside
    window.addEventListener('click', (e) => {
        if (e.target.classList.contains('popup-overlay')) {
            hidePopup(loginPopup);
            hidePopup(registerPopup);
        }
    });
});
</script>

</html>
